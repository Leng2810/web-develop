...
class Parameters<T> {
  ...
  static _isPrimitive(value: any) {
    return Parameters.PRIMITIVE_TYPES.some((Primitive) =>
      [typeof value, String(value)].some(
        (val) => val === Primitive.name.toLowerCase(),
      ),
    );
  }
  ...
  public require(key: string) {
    ...
    if (!(param instanceof Parameters)) {
      throw new Error(`param ${key} is not a Parameters instance`);
    }
    return param;
  }
    ...
    for (const key in filters) {
      ...
      if (_.isArray(filtersArray) && (param = this._fetch(key))) {
        ...
        if (
          ...
          (isArrObj = Object.keys(param._params).every(
            (i) => !_.isNaN(Number(i)),
          ))
        ) {
          if (isArrObj) {
            params[key] = _.transform(
              param._params,
              (result: { [key: string]: any }, value, key: string) => {
                result[key] = Parameters._isPrimitive(value)
                  ? value
                  : value.permit(filtersArray).value();
              },
              {},
            );
          } else if (!param._params.some(Parameters._isPrimitive)) {
            params[key] = param._params.map((p: any) =>
              p.permit(filtersArray).value(),
            );
          } else {
            params[key] = param._params.filter((p: any) =>
              Parameters._isPrimitive(p),
            );
          }
          continue;
        }
