...
export const sendError = (
  ...
  if (errorSubject instanceof AggregateError) {
    const validationErrorItems = errorSubject.errors
      .map((errorGroups: any) => errorGroups.errors)
      .map((singleError) => singleError.errors);
    return res
      .status(422)
      .json({ error: FailValidation(validationErrorItems.concat()) });
  }
